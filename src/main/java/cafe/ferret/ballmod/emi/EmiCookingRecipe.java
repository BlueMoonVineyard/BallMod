...
public class EmiCookingRecipe implements EmiRecipe {
	...
	public EmiCookingRecipe(Identifier id, EmiIngredient inputs, EmiStack output, EmiRecipeCategory category, int cookTime, float experience, int fuelMultiplier, boolean infiniBurn) {
		this.id = id;
	...
	public void addWidgets(WidgetHolder widgets) {
		widgets.addFillingArrow(24, 5, 50 * cookTime).tooltip((mx, my) -> {
			return List.of(TooltipComponent.of(EmiPort.ordered(EmiPort.translatable("emi.cooking.time", cookTime / 20f))));
		});
		if (infiniBurn) {
